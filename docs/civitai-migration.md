...
pnpm add @civitai/generation-metadata@file:../media-metadata

# or a workspace-style link for live iteration
pnpm link ../media-metadata
```

Once published, it's just `pnpm add @civitai/generation-metadata`.
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/utils/media-preprocessors/image.preprocessor.ts:11` (upload pipeline)                                                                                                                    | `getMetadata(file)`                   | `(await readCivitaiMetadata(file)).raw` — optionally re-validated with the app's `imageMetaSchema` if the stricter `extra` shape must be preserved (see README "Known deliberate differences") |
| `src/utils/metadata/index.ts` `ExifParser(file)` → `parse/getMetadata/isMadeOnSite/encode` (GenerationForm.tsx:2719, MetadataExtractionPanel.tsx:222, PanelModal.tsx:499, metadata-test page) | one closure-returning parser          | `const md = await readCivitaiMetadata(file)` → `md.raw`, `md.civitai.madeOnSite`; `encodeMetadata(md.raw)`                                                                                     |
...
| `src/components/generation_v2/inputs/PromptInput.tsx:65` (paste parameters)                                                                                                                   | `parsePromptMetadata(text)`           | `parseGenerationText(text, { plugins: [civitai()] }).raw` — example 04                                                                                                                         |
...
| `src/utils/metadata/index.ts:64` `isMadeOnSite`                                                                                                                                               | `Artist === 'ai'` check               | `md.civitai.madeOnSite`, or `isMadeOnSite(exif)` from `@civitai/generation-metadata/civitai`                                                                                                        |
| `src/server/schema/image.schema.ts` `imageMetaSchema`                                                                                                                                         | app schema validates parser output    | the package validates with its own vendored `generationMetadataSchema`; the app schema can derive from it (`.omit()` the package's loose `extra` FIRST, then extend with the app's stricter `extra` shape and `unmatchedResources` — a plain `.extend()` silently unions the two `extra` types), or keep re-validating `md.raw`. Gate this on the package being published: the app's core DB schema should not depend on a `file:` sibling-directory dep                 |
...
- The app's `getMetadata()` catch-all (`{}` on any failure) is matched by `md.raw` being `{}` (and `md.civitai.generation` absent)
  when nothing parses.
- `extra` stripping: the package keeps the full `extra` record where the old `getMetadata()`
  ...
  re-validate `md.raw` with `imageMetaSchema` at the boundary (one line, same behavior as today).
